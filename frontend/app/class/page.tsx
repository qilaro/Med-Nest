...
async function getClassBrands(page: number = 1, searchQuery?: string) {
  ...
  const [data, totalResult] = await Promise.all([
    db.execute(sql`
      ...
      FROM brands b
      WHERE b.brand_name ILIKE ${searchCondition} OR b.generic_name ILIKE ${searchCondition} OR b.therapeutic_class ILIKE ${searchCondition}
      ORDER BY b.therapeutic_class ASC, b.brand_name ASC
...
export default async function ClassPage(props: { searchParams: Promise<{ page?: string, q?: string }> }) {
  const searchParams = await props.searchParams;
  const page = parseInt(searchParams.page || '1');
  const query = searchParams.q || '';
  const { list: brandList, totalPages } = await getClassBrands(page, query);

  return (
    <div className="container-medq py-10">
      <div className="max-w-5xl mx-auto">
        <header className="mb-8">
          <h1 className="text-3xl font-bold text-navy mb-2">Browse Drug Classes</h1>
          <p className="text-muted-foreground">Browse medications organized by therapeutic classification.</p>
        </header>

        <form className="flex flex-wrap gap-4 mb-10 items-center" action="/class" method="GET">
          <div className="relative flex-1">
            ...
            <input name="q" defaultValue={query} type="text" className="w-full border border-gray-200 bg-transparent px-10 py-4 h-14 rounded-xl text-base shadow-sm outline-none focus:border-primary" placeholder="Search by drug class, brand, or generic..." />
          </div>
